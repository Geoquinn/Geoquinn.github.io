...
let houseZoomTransform={x:0,y:0,k:1};
...
const initializeHouseZoom=()=>{
  const svg=document.querySelector('.district-map'),indicator=document.querySelector('#map-zoom-level');
  if(!svg||!window.d3)return;
  const selection=d3.select(svg),layer=selection.select('.district-map-layer');
  const zoom=d3.zoom().scaleExtent([1,12]).extent([[0,0],[960,600]]).translateExtent([[-90,-90],[1050,690]]).on('zoom',event=>{
    houseZoomTransform={x:event.transform.x,y:event.transform.y,k:event.transform.k};
    layer.attr('transform',event.transform.toString());
    if(indicator)indicator.textContent=`${Math.round(event.transform.k*100)}%`;
    svg.classList.toggle('is-zoomed',event.transform.k>1.01);
  });
  selection.call(zoom);
  const saved=d3.zoomIdentity.translate(houseZoomTransform.x,houseZoomTransform.y).scale(houseZoomTransform.k);
  selection.call(zoom.transform,saved);
  document.querySelectorAll('[data-map-zoom]').forEach(button=>button.addEventListener('click',()=>{
    const action=button.dataset.mapZoom;
    if(action==='reset')selection.call(zoom.transform,d3.zoomIdentity);
    else selection.call(zoom.scaleBy,action==='in'?1.5:1/1.5);
  }));
};

const initializeTossupScroll=()=>{
  const track=document.querySelector('.tossup-race-track');
  if(!track)return;
  document.querySelectorAll('[data-tossup-scroll]').forEach(button=>button.addEventListener('click',()=>{
    const distance=Math.max(300,track.clientWidth*.78)*(button.dataset.tossupScroll==='next'?1:-1);
    if(typeof track.scrollBy==='function')track.scrollBy({left:distance,behavior:'smooth'});
    else track.scrollLeft+=distance;
  }));
};
...
const renderHouse=()=>{
  cancelDistrictReveal();
  const stage=document.querySelector('.output-stage'),standardRaceSection=document.querySelector('.race-section');
  standardRaceSection.hidden=true;
  standardRaceSection.id='national-race-board';
  if(!houseCollection||!houseModelData){
    stage.innerHTML=`<div class="output-head"><div><p class="section-index">Model 1 / Special elections</p><h3>Run the national House model</h3><p>Loading district baselines, demographics, and boundaries.</p></div><div class="office-chip">U.S. House</div></div><div class="map-status">Loading Model 1 data…</div>`;
    loadHouseData();
    return;
  }

  const output=houseMode==='blank'?null:simulateHouse();
  const results=output?output.results:[],resultById=new Map(results.map(result=>[result.id,result])),districtById=new Map(houseModelData.districts.map(district=>[district.id,district]));
  const selected=selectedDistrict?resultById.get(selectedDistrict):null,selectedBase=selectedDistrict?districtById.get(selectedDistrict):null,signal=weightedSignal(houseModelData.cases,houseHalfLife,houseIncludeStatewide),medianGop=output?435-output.medianDem:0;
  const projection=d3.geoAlbersUsa().fitExtent([[18,18],[942,582]],houseCollection),path=d3.geoPath(projection);
  const mapPaths=houseCollection.features.map(feature=>{
    const id=feature.properties.GEOID,result=resultById.get(id),district=districtById.get(id);
    return `<path d="${path(feature)||''}" class="district-shape ${result?result.ratingClass:'unrated'} ${output?'district-loading':''} ${selectedDistrict===id?'selected':''}" data-district="${id}"><title>${result?`${result.label}: ${result.rating}, ${probabilityLabel(result.demWinProbability)} Democratic win probability, ${marginLabel(result.projectedMargin)} projected margin`:`${district?district.label:id}: run the model`}</title></path>`;
  }).join('');
  ...
  const tossups=[...results].filter(result=>result.ratingClass==='toss').sort((a,b)=>Math.abs(a.demWinProbability-.5)-Math.abs(b.demWinProbability-.5));

  const detail=!selectedDistrict
    ?`<div class="detail-empty"><strong>Click any district</strong><p>Its baseline, simulated vote, uncertainty, and demographic inputs will appear here.</p></div>`
    :!output
      ?`<div class="detail-empty"><span>${selectedBase.state}</span><strong>${selectedBase.label}</strong><p>The map is blank. Run Model 1 to produce this district's probability estimate.</p></div>`
      :`<div class="detail-result model1-detail"><span>${selected.state} · ${selected.incumbent?`${selected.incumbent} (${selected.incumbentParty||'?'})`:'Open seat'}</span><h4>${selected.label}</h4><div class="detail-rating ${selected.ratingClass}">${selected.rating}</div><div class="model1-probability"><span>Democratic win probability</span><strong>${probabilityLabel(selected.demWinProbability)}</strong><i><b style="width:${selected.demWinProbability*100}%"></b></i></div><dl><div><dt>Democratic</dt><dd>${selected.demShare.toFixed(1)}% <small>${selected.demVotes.toLocaleString()}</small></dd></div><div><dt>Republican</dt><dd>${selected.gopShare.toFixed(1)}% <small>${selected.gopVotes.toLocaleString()}</small></dd></div><div><dt>Other</dt><dd>${selected.otherShare.toFixed(1)}% <small>${selected.otherVotes.toLocaleString()}</small></dd></div></dl><div class="detail-margin"><span>Mean simulated margin</span><strong>${marginLabel(selected.projectedMargin)}</strong></div><div class="model1-interval"><span>Approx. 80% district interval</span><strong>${marginLabel(selected.projectedMargin-1.282*selected.marginSd)} to ${marginLabel(selected.projectedMargin+1.282*selected.marginSd)}</strong></div><div class="model1-baselines"><div><span>2024 president</span><strong>${marginLabel(selected.margin)}</strong></div><div><span>2022 House</span><strong>${selected.house2022?marginLabel(selected.house2022.margin):'Not comparable'}</strong></div><div><span>Demographic adjustment</span><strong>${selected.demographicsComparable?marginLabel(selected.demographicAdjustment):'National effect only'}</strong></div></div>${selected.demographicsComparable&&selected.demographics?`<div class="model1-demographics"><span>${selected.demographicsMethod==='2024-ACS-small-area-reaggregation'?'2020–2024 ACS · rebuilt for 120th lines':'2020–2024 ACS district profile'}</span><div><b>${selected.demographics.age65.toFixed(1)}%<small>Age 65+</small></b><b>${selected.demographics.seniorBachelors.toFixed(1)}%<small>Senior BA+</small></b><b>${selected.demographics.whiteNH.toFixed(1)}%<small>White, non-Hisp.</small></b><b>${selected.demographics.hispanic.toFixed(1)}%<small>Hispanic</small></b></div></div>`:`<p class="provisional-note">A usable ACS profile is unavailable for this district, so the model applies only the national effect.</p>`}<p>Expected turnout ${selected.totalVotes.toLocaleString()} · about 72% of the district's 2024 presidential vote.</p></div>`;

  const tossupCards=tossups.map(race=>{
    const nominee=houseNomineesById.get(race.id)||{},democratic=escapeHtml(nominee.democratic||'Nominee pending'),republican=escapeHtml(nominee.republican||'Nominee pending');
    return `<button type="button" class="tossup-race-card ${selectedDistrict===race.id?'active':''}" data-tossup-race="${race.id}" aria-pressed="${selectedDistrict===race.id}"><span class="tossup-card-top"><strong>${race.label}</strong><b class="${race.projectedMargin>=0?'dem-text':'gop-text'}">${marginLabel(race.projectedMargin)}</b></span><span class="tossup-card-meta">${race.rating} · ${probabilityLabel(race.demWinProbability)} Democratic win probability</span><span class="tossup-candidate"><i class="dem-text">D</i><b>${democratic}</b><em>${race.demShare.toFixed(1)}% · ${race.demVotes.toLocaleString()}</em></span><span class="tossup-candidate"><i class="gop-text">R</i><b>${republican}</b><em>${race.gopShare.toFixed(1)}% · ${race.gopVotes.toLocaleString()}</em></span><span class="tossup-card-foot">Other ${race.otherShare.toFixed(1)}% · ${race.totalVotes.toLocaleString()} projected votes</span></button>`;
  }).join('');
  const tossupRail=`<section class="tossup-races" id="race-board" aria-labelledby="tossup-races-title"><div class="tossup-races-head"><div><span>Special Elections model · decisive races</span><h4 id="tossup-races-title">Toss-up district results</h4><p>${output?`${tossups.length} district${tossups.length===1?'':'s'} currently fall inside the 35–65% Democratic win-probability band.`:'Run Model 1 to identify and compare the districts most likely to decide House control.'}</p></div>${output&&tossups.length?`<div class="tossup-scroll-controls" aria-label="Scroll toss-up districts"><button type="button" data-tossup-scroll="previous">← Previous</button><button type="button" data-tossup-scroll="next">Next →</button></div>`:''}</div>${output?(tossups.length?`<div class="tossup-race-track" tabindex="0" aria-label="Scrollable toss-up district simulation results">${tossupCards}</div>`:`<div class="tossup-empty"><strong>No toss-ups in this run</strong><p>Run the model again or change its settings to redraw the national environment.</p></div>`):`<div class="tossup-empty"><strong>Awaiting a simulation</strong><p>The district results rail will populate after the Special Elections model runs.</p></div>`}</section>`;
  ...
  if(output){
    const mapWrap=document.querySelector('.district-map-wrap');
    if(mapWrap)mapWrap.insertAdjacentHTML('afterbegin',`<div class="district-load-progress" role="status"><span>Loading district outcomes</span><strong>0 / ${houseCollection.features.length}</strong><i><b></b></i></div>`);
  }
  ...
  document.querySelectorAll('[data-tossup-race]').forEach(button=>button.addEventListener('click',()=>{selectedDistrict=button.dataset.tossupRace;renderHouse();}));
  initializeHouseZoom();
  initializeTossupScroll();
  renderBatchTracker();
...
const update=()=>{
  ...
  document.querySelector('.race-section').id='race-board';

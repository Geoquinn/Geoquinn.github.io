...
The House map now occupies a larger share of the desktop simulator and supports
wheel, trackpad, pinch, button, and drag navigation. Use Zoom in, Zoom out, or
Reset map to inspect compact urban districts. After a simulation, a horizontal
toss-up district rail lists every race in the model's 35-65 percent probability
band with both available nominees, projected vote shares, raw vote estimates,
win probability, and projected margin.

The Senate and Governors routes now run the same Special Elections signal
through office-specific Bayesian simulations. Visitors can run 100, 500,
1,000, 2,500, or 5,000 elections. Each page reports the median outcome, an
80-percent outcome range, the probability of Democratic control or a majority,
the posterior national shift, a clickable 2026 state map, race-level win
probabilities, candidate matchups, projected vote totals, and predictive
intervals. Outcomes inside each simulated election share national and office
shocks so the statewide results remain correlated.
